...
class ArchiverPlugin extends Plugin {
	...
	private function archive(Ticket $ticket) {
		...
		$date = DateTime::createFromFormat('Y-m-d H:i:s', $ticket->getCreateDate ())->format('Y/m/d');
		$path = sprintf("%s/%s", $this->getConfig()->get('archive-path'), $date);
		...
		if ($this->getConfig ()->get ( 'mode' ) == 'basic') {
			...
			if (self::DEBUG)
				error_log ( "Exported: " . $number . ' Date: ' . $date . ' Subject: ' . $subject);
			return;
